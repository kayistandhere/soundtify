    ...
    <table class="table table-borderless text-white">
      ...
      <tbody v-if="!this.isLoading">
        <tr class="fs-9 text-white table-items-hover" id="item" :key="song" v-for="song in this.songData">
          ...
          <th scope="row" class="custom-cursor"><span class="material-symbols-rounded"
              @click="addSong(song)">play_arrow</span></th>
          <td>
            <div class="custom_card d-flex align-items-center">
              ...
              <div class="">
                ...
                <div class="d-flex align-items-center">
                  <span class=" fs-9">{{ this.artistNameMap.get(song.artistId) }}</span>
                </div>
              </div>
            </div>
          </td>
...
export default {
  ...
  methods: {
    ...
    async getArtistName(){
      for (const song of this.songData) {
        let artist = await getArtistById(song.artistId);
        if(artist == null) continue;
        this.artistNameMap.set(artist.id, artist.name);
      }
    },
